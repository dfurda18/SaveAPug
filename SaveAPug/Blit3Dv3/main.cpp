...
Sprite *backgroundGameplay = NULL;
Sprite *dizzyPug = NULL;
Sprite *happyPug = NULL;
Sprite *hurtPug = NULL;
Sprite *standingPug = NULL;
Sprite *logo = NULL;
Sprite *GameOverTitle = NULL;
Sprite *start = NULL;
Sprite *ballSprite[4];
Sprite *blueBrickSprite[6];
Sprite *greenBrickSprite[6];
Sprite *greyBrickSprite[6];
Sprite *blackBrickSprite[6];
Sprite *redBrickSprite[6];
Sprite *yellowBrickSprite[6];
Sprite *paddleSpriteLeft = NULL;
Sprite *paddleSpriteRight = NULL;
Sprite *paddleSpriteCenter = NULL;
Sprite *pugFace = NULL;
Sprite *kong = NULL;
Sprite *livesSprite = NULL;


Sprite * powerUpSprites[5];
...
std::vector<std::string> split(string str, char separator)
{
...
void MakeLevel()
{
	...
	currentBallSpeed = 80 + 10 * level; //balls move faster by default at higher levels
	...
	std::ifstream fileName("Levels\\" + std::to_string(level) + ".txt");
	std::string newLine;
	...
	if (fileName >> newLine)
	{
		numberOfLines = stoi(newLine);
	}
	if (fileName >> newLine)
	{
		trappedPugs = stoi(newLine);
	}
	if (numberOfLines > 0)
	{
		for (int linesCounter = 0; linesCounter < numberOfLines; linesCounter++)
		{
			if (fileName >> type >> x >> y)
			{
				BrickEntity* brick = MakeBrick(getColour(stoi(type)), stoi(x), stoi(y));
				brickEntityList.push_back(brick);
				if (brickRandom(rng))
				{
...
void Init()
{
	...
	debugFont = blit3D->MakeAngelcodeFontFromBinary32("Media\\Fonts\\ConcertOne.bin");

	//load the sprites
	for (int spriteCounter = 0; spriteCounter < 6; spriteCounter++)
	{
		blueBrickSprite[spriteCounter] = blit3D->MakeSprite(spriteCounter * 64, 0, 64, 32, "Media\\TileSet.png");
	}
	for (int spriteCounter = 0; spriteCounter < 6; spriteCounter++)
	{
		greenBrickSprite[spriteCounter] = blit3D->MakeSprite(spriteCounter * 64, 32, 64, 32, "Media\\TileSet.png");
	}
	for (int spriteCounter = 0; spriteCounter < 6; spriteCounter++)
	{
		greyBrickSprite[spriteCounter] = blit3D->MakeSprite(spriteCounter * 64, 64, 64, 32, "Media\\TileSet.png");
	}
	for (int spriteCounter = 0; spriteCounter < 6; spriteCounter++)
	{
		blackBrickSprite[spriteCounter] = blit3D->MakeSprite(spriteCounter * 64, 96, 64, 32, "Media\\TileSet.png");
	}
	for (int spriteCounter = 0; spriteCounter < 6; spriteCounter++)
	{
		redBrickSprite[spriteCounter] = blit3D->MakeSprite(spriteCounter * 64, 128, 64, 32, "Media\\TileSet.png");
	}
	for (int spriteCounter = 0; spriteCounter < 6; spriteCounter++)
	{
		yellowBrickSprite[spriteCounter] = blit3D->MakeSprite(spriteCounter * 64, 160, 64, 32, "Media\\TileSet.png");
	}
	//power up animated sprite lists
	powerUpSprites[0] = blit3D->MakeSprite(128, 192, 64, 32, "Media\\TileSet.png");
	powerUpSprites[1] = blit3D->MakeSprite(192, 192, 64, 32, "Media\\TileSet.png");
	powerUpSprites[2] = blit3D->MakeSprite(256, 192, 64, 32, "Media\\TileSet.png");
	powerUpSprites[3] = blit3D->MakeSprite(320, 192, 64, 32, "Media\\TileSet.png");
	powerUpSprites[4] = blit3D->MakeSprite(0, 192, 64, 32, "Media\\TileSet.png");

	logo = blit3D->MakeSprite(0, 0, 616, 423, "Media\\Title.png");
	GameOverTitle = blit3D->MakeSprite(0, 0, 1021, 150, "Media\\GameOver.png");

	paddleSpriteLeft = blit3D->MakeSprite(192, 192, 32, 32, "Media\\TileSet.png");
	paddleSpriteRight = blit3D->MakeSprite(224, 192, 32, 32, "Media\\TileSet.png");
	paddleSpriteCenter = blit3D->MakeSprite(192, 192, 64, 32, "Media\\TileSet.png");

	for (int spriteCounter = 0; spriteCounter < 4; spriteCounter++)
	{
		ballSprite[spriteCounter] = blit3D->MakeSprite(64 + spriteCounter * 32, 192, 32, 32, "Media\\TileSet.png");
	}
	...
	// Backgrounds
	background1 = blit3D->MakeSprite(0, 0, 1920, 1080, "Media\\Background.png");
	backgroundGameplay = blit3D->MakeSprite(0, 0, 1920, 1080, "Media\\BackgroundGameplay.png");

	// Pugs
	dizzyPug = blit3D->MakeSprite(0, 0, 427, 559, "Media\\DizzyPug.png");
	happyPug = blit3D->MakeSprite(0, 0, 494, 646, "Media\\HappyPug.png");
	hurtPug = blit3D->MakeSprite(0, 0, 429, 677, "Media\\HurtPug.png");
	standingPug = blit3D->MakeSprite(0, 0, 485, 687, "Media\\StandingPug.png");

	// Titles
	start = blit3D->MakeSprite(0, 0, 231, 85, "Media\\Start.png");
	...
	BallEntity *ball = MakeBall(ballSprite[0]);
	...
	paddleEntity = MakePaddle(blit3D->screenWidth / 2, 60, paddleSpriteCenter);
...
void Update(double seconds)
{
	...
	if (brickEntityList.size() == 0 || trappedPugs <= 0)
	{
		level++;
		if (level > 3)
		{
	...
	while (elapsedTime >= timeStep)
	{
		...
		{
			b2Vec2 velocity = b->body->GetLinearVelocity();
			...
			if (velocity.y > -0.5f && velocity.y < 0.5f)
			{
				...
				velocity.Normalize();
				velocity *= speed;
				b->body->SetLinearVelocity(velocity);
			}
			b->Update(timeStep);
		}
			...
			if (A != NULL && B != NULL) //if there is an entity for these objects...
			{
				...
				if (B->typeID == ENTITYBALL && A->typeID == ENTITYBRICK)
				{
					...
					if (be->HandleCollision())
					{
						...
						if (be->colour == BrickColour::RED)
						{
							b2Vec2 position = be->body->GetPosition();
							position = Physics2Pixels(position);
							PowerUpEntity* p = MakePowerUp(PowerUpType::EXTRA_LIFE, position.x, position.y);
							entityList.push_back(p);
						}
						if (be->colour == BrickColour::BLACK)
						{
							b2Vec2 position = be->body->GetPosition();
							position = Physics2Pixels(position);
							PowerUpEntity* p = MakePowerUp(PowerUpType::TRAPPED_PUG, position.x, position.y);
							entityList.push_back(p);
						}
					...
					if (be->colour == BrickColour::YELLOW)
					{
						score += 30;
					}
						...
						case PowerUpType::MULTIBALL:
							for (int j = ballEntityList.size() - 1; j >= 0; --j)
							{
								...
								for (int i = 0; i < 2; ++i)
								{
									...
									{
										BallEntity *b = MakeBall(ballSprite[0]);
...
void Draw(void)
{
	...
	std::string pugString;
	std::string scoreString;
	switch (gameState)
	{
	case START:
		background1->Blit(blit3D->screenWidth / 2, blit3D->screenHeight / 2);
		happyPug->Blit(247+80, 323+40);
		standingPug->Blit(1920-80-243, 343+40);
		logo->Blit(blit3D->screenWidth / 2, blit3D->screenHeight / 2 + 200);
		start->Blit(blit3D->screenWidth / 2, blit3D->screenHeight / 2 - 200);
		break;

	case PLAYING:
		backgroundGameplay->Blit(blit3D->screenWidth / 2, blit3D->screenHeight / 2);
		...
		debugFont->BlitText(40, 1050, lifeString);
		pugString = "Remaining Pugs: " + std::to_string(trappedPugs);
		debugFont->BlitText(1250, 1050, pugString);
		scoreString = "Score: " + std::to_string(score);
		debugFont->BlitText(blit3D->screenWidth / 2-180, 1050, scoreString);
		break;
	case GAME_OVER:
		background1->Blit(blit3D->screenWidth / 2, blit3D->screenHeight / 2);
		dizzyPug->Blit(247 + 80, 323 + 40);
		hurtPug->Blit(1920 - 80 - 243, 343 + 40);
		scoreString = "Final Score: " + std::to_string(score);
		debugFont->BlitText(blit3D->screenWidth / 2 - 230, 400, scoreString);
		GameOverTitle->Blit(blit3D->screenWidth / 2, blit3D->screenHeight / 2 + 350);
		break;
	}
